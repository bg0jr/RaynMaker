...
namespace RaynMaker.Modules.Import.ScenarioTests
{
    ...
    class LoadingDataSourcesScenarios : TestBase
    {
        ...
        [Test]
        public void Load_ImportSpecVersion2_AllDataDeserialized()
        {
            using( var stream = new FileStream( Path.Combine( TestDataRoot, "DataSources", "Version2.xdb" ), FileMode.Open, FileAccess.Read ) )
            {
                var serializer = new ImportSpecSerializer();
